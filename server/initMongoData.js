var db = db.getSiblingDB('casemanager');
var cases = [
  "Apple Bees",
  "Bacon Trends",
  "Chocolate Fans",
...
for (i = 0; i < l; i++) {
  var acase = cases[i];
  var nameGenderHost = caseNames[i].split(',');
  ...
  firstOrder = Math.floor(Math.random() * entries.length);
  lastOrder = Math.floor(Math.random() * entries.length);
  ...
  for (j = firstOrder; j <= lastOrder && j < n; j++) {
    ...
    var o = {
      ...
      "price":    entries[j].price,
      "quantity": entries[j].quantity,
      "date":     tomorrow
    };
    c.entries.push(o);
  }
  c.orderCount = c.entries.length;
  db.cases.insert(c);
}
db.settings.remove({});
var r = {'nextSeqNumber': 24, 'collectionName': "cases"};
